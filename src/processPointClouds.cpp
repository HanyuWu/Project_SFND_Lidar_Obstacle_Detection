...
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::SegmentPlane(typename pcl::PointCloud<PointT>::Ptr cloud, int maxIterations, float distanceThreshold)
{
  ...
  {
    std::cout << "We could not find a plannar model for the given dataset." << std::endl;
  }
...
template <typename PointT>
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::RansacPlane(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, int maxIterations, float distanceTol)
{
...
template <typename PointT>
void ProcessPointClouds<PointT>::Proximity(int indice, typename pcl::PointCloud<PointT>::Ptr cloud, std::vector<int> &cluster, std::vector<bool> &processed, KdTree *tree, const float distanceTol)
{
...
std::vector<typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::euclideanCluster(typename pcl::PointCloud<PointT>::Ptr cloud, KdTree *tree, float distanceTol, int minSize, int maxSize)
{
  ...
  while (i < cloud->points.size())
  {
    ...
    {
      typename pcl::PointCloud<PointT>::Ptr clusterPoint(new pcl::PointCloud<PointT>);
      for (auto index : cluster)
      {
        clusterPoint->points.push_back(cloud->points[index]);
      }
      clusterPoint->width = clusterPoint->points.size();
      clusterPoint->height = 1;
      clusters.push_back(clusterPoint);
    }
  }
  return clusters;
}